...
const {getMetadata} = require("lib/MetadataProvider");
  ...
  saveMetadata(data) {
    getMetadata(data)
      .then(result => {
        ...
        const sanitizedURL = this._sanitizeURL(result.url);
        const cacheKey = this._createCacheKey(sanitizedURL);
        ss.storage.embedlyData[cacheKey] = Object.assign({}, ss.storage.embedlyData[cacheKey], result);
        ss.storage.embedlyData[cacheKey].refreshTime = currentTime;
        ss.storage.embedlyData[cacheKey].accessTime = currentTime;
      })
