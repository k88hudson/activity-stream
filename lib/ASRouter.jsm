...
class _ASRouter {
  ...
  overrideOrEnableLegacyOnboarding() {
    ...
    if (!allowLegacyOnboarding && onboardingEnabled === true) {
      Services.prefs.setBoolPref(ONBOARDING_FINISHED_PREF, false);
    } else if (allowLegacyOnboarding && onboardingEnabled === false) {
  ...
  async onPrefChange() {
    ...
    this.dispatchToAS({type: at.AS_ROUTER_PREF_CHANGED, data: ASRouterPreferences.specialConditions});
  }
  ...
  async init(channel, storage, dispatchToAS) {
    ...
    this.dispatchToAS({type: at.AS_ROUTER_INITIALIZED, data: ASRouterPreferences.specialConditions});
