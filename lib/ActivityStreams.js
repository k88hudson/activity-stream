...
require("seedrandom"); //require for the side effect
...
function ActivityStreams(options = {}) {
  ...
  Math.seedrandom(options.clientID);
  if (Math.random() <= 0.1) {
    // TODO: enable the experiment for this client
    console.log("enabling the experiment");
  }
...
ActivityStreams.prototype = {
  ...
  _respondToExperimentsRequest(msgName, params) {
    let {worker} = params;
    /// TODO: Add real experiment data
    const experimentData = {};
    this.send(am.actions.Response("SEARCH_STATE_REQUEST", experimentData), worker);
  },
