...
ActivityStreams.prototype = {
  ...
  _processAndSendLinks(links, responseType, worker, options) {
    ...
    // }
    this._previewProvider.getEnhancedLinks(processedLinks, previewsOnly)
      .then(cachedLinks => {
  ...
  _setupPageMod() {
    ...
    this._metadataPagemod = new PageMod({
      include: "*",
      attachTo: ["existing", "top"],
      contentScriptFile: data.url("metadata-bridge.js"),
      onAttach: worker => {
        worker.port.on("metadata-full-text", data => {
          this._previewProvider.saveMetadata(data);
          console.log('saving...');
        });
      }
    });
